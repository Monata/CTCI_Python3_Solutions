...
class BinaryTree:
    ...
    def find_sum_helper(self, node, target_sum, count):
        if node:
            sums = []
            ls = self.find_sum_helper(node.right,target_sum,count)
            rs = self.find_sum_helper(node.left,target_sum,count)
            if ls:
                sums.extend(ls)
            if rs:
                sums.extend(rs)
            sums.append(0)
            sums = [i + node.data for i in sums]

            res = [1 for i in sums if i == target_sum]
...
if __name__ == "__main__":
    print("find_sum") ##all nums
    ran = range(-2,7)
    test_count = Counter()
    t = BinaryTree([3])
    t.root = TreeConstructor(ran)
    print(t)
    for i in ran:
        res = t.find_sum(i)
        test_count[i] = res
